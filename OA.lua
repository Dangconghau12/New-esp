-- ZilpX Menu ESP VIP - Drag menu, key giữa, đủ chức năng VIP
...
local keyUnlocked, menuActivated = false, false
local menuCircleBtn, menuFrame, dragBtn, featureObjs = nil, nil, nil, {}
local posList = {"left","right","top","bottom"} local posIndex = 1
local menuPositions = {
...
local State = {
    ...
    fov=110, hitboxSize=6, hitboxHeadSize=8, highlight=true,
    showTeam=true, showWeapon=true, menu=false, menupos="left"
}
local ESPMap = {}
...
task.spawn(function() while true do updateESP() wait(1.2) end end)
RunService.RenderStepped:Connect(function()
    for p,obj in pairs(ESPMap) do
        local char = obj.char
        if obj.bb then obj.bb.Enabled = State.esp and keyUnlocked and menuActivated end
        if obj.highlight then obj.highlight.Enabled = State.highlight and keyUnlocked and menuActivated end
...
-- UI có nút kéo menu (drag menu), key ở giữa, tên menu ZilpX Menu
local function makeFeature(obj)
    table.insert(featureObjs, obj)
    obj.Visible = false
end
local function setFeaturesVisible(v) for _,obj in ipairs(featureObjs) do obj.Visible = v end end
local function createUI()
    ...
    menuFrame.Size = UDim2.fromOffset(250,390)
    menuFrame.Position = UDim2.new(0.5,-125,0.5,-195)
    menuFrame.BackgroundColor3 = Color3.fromRGB(35,40,60)
    menuFrame.BorderSizePixel = 0
    menuFrame.Visible = false
    menuFrame.Active = true
    menuFrame.Draggable = false -- Drag bằng nút riêng
    ...
    dragBtn.Size = UDim2.new(0,60,0,22)
    dragBtn.Position = UDim2.new(1,-70,0,8)
    dragBtn.BackgroundColor3 = Color3.fromRGB(35,120,180)
    dragBtn.TextColor3 = Color3.new(1,1,1)
    dragBtn.Font = Enum.Font.GothamBold
    dragBtn.TextSize = 13
    dragBtn.Text = "Kéo menu"
    ...
    dragBtn.MouseButton1Down:Connect(function(input)
        ...
        connMove = mouse.Move:Connect(function()
            if dragging then
                local delta = Vector2.new(mouse.X, mouse.Y) - startMouse
                menuFrame.Position = UDim2.new(0, startPos.X.Offset+delta.X, 0, startPos.Y.Offset+delta.Y)
            end
        end)
        connUp = mouse.Button1Up:Connect(function()
            dragging = false
    ...
    keyBox.FocusLost:Connect(function()
        if keyBox.Text == keyRequired then
            ...
            StarterGui:SetCore("SendNotification", {Title="ZilpX Menu",Text="Đã nhập đúng key!",Duration=3})
        else
            keyUnlocked = false
            ...
            StarterGui:SetCore("SendNotification", {Title="ZilpX Menu",Text="Sai key, hãy nhập lại!",Duration=3})
        end
    end)
    ...
    makeToggle(254, "Hiện Team", function() return State.showTeam end, function(v) State.showTeam = v end)
    makeToggle(288, "Hiện Vũ Khí", function() return State.showWeapon end, function(v) State.showWeapon = v end)
    makeNumberBox(322, "FOV", function() return State.fov end, function(v) State.fov = v end, 30, 300)
    makeNumberBox(356, "Hitbox Thân", function() return State.hitboxSize end, function(v) State.hitboxSize = v end, 2, 20)
    makeNumberBox(356+34, "Hitbox Đầu", function() return State.hitboxHeadSize end, function(v) State.hitboxHeadSize = v end, 2, 20)
end
