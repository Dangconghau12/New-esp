-- CONG HAU ESP - Menu tròn đổi vị trí, menu frame draggable, nhập key unlock
-- Dangconghau12 - 2025-08-16
...
local function setMenuPosition(pos)
    ...
    -- KHÔNG di chuyển menuFrame, chỉ di chuyển menuCircleBtn
end
...
local function createUI()
    ...
    keyBox.FocusLost:Connect(function()
        ...
        for _,v in pairs(menuFrame:GetChildren()) do
            if v.Name ~= "KeyLabel" and v.Name ~= "KeyBox" and v ~= enterBtn and v ~= moveKeyBtn then
                v.Visible = false
            end
        end
    end)
